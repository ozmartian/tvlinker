...
class Updater(QThread):
    ...
    pypi_api_endpoint = 'https://pypi.python.org/pypi/tvlinker/json'
    github_api_endpoint = 'https://api.github.com/repos/ozmartian/tvlinker/releases/latest'
    latest_release_webpage = 'https://github.com/ozmartian/tvlinker/releases/latest'
    ...
    def install_update(self, parent: QWidget) -> None:
        returncode = self.cmd_exec('x-terminal-emulator', '-title "tvlinker Updater" -e "sudo pip3 install '
                                   + '--upgrade tvlinker"')
    ...
    def notify_no_update(parent: QWidget) -> None:
        mbox = QMessageBox(parent)
        mbox.setIconPixmap(icon('fa.thumbs-up', color='#6A687D').pixmap(64, 64))
        mbox.setWindowTitle('%s UPDATER' % qApp.applicationName())
        mbox.setText('<h3 style="color:#6A687D;">%s %s</h3>'
                     % (qApp.applicationName(), qApp.applicationVersion()))
        mbox.setInformativeText(
            'You are already running the latest version.<table width="350"><tr><td></td></tr></table>')
    ...
    def notify_restart(parent: QWidget) -> bool:
        ...
        mbox.setText('<h3 style="color:#6A687D;">INSTALLATION COMPLETE</h3>' +
